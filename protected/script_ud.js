...
// ===================================================================
// Logic:
// - Nếu BẮT ĐƯỢC "Remaining Previews" → Kiểm tra xem có đủ không
// - Nếu KHÔNG BẮT ĐƯỢC → Cho phép tiếp tục (tài khoản unlimited)
// ===================================================================
...
function validateChunksBeforeGeneration() {
    ...
    // ============================================================
    // TRƯỜNG HỢP 1: KHÔNG BẮT ĐƯỢC THÔNG TIN
    // → CHO PHÉP TIẾP TỤC (không giới hạn)
    // ============================================================
    if (remaining === null) {
        addLogEntry('⚠️ Không tìm thấy thông tin "Remaining Previews" trên trang', 'warning');
        addLogEntry('✅ CHO PHÉP TIẾP TỤC TẠO (chế độ unlimited)', 'success');
        return true; // ← KHÔNG CHẶN
    }
    
    // ============================================================
    // TRƯỜNG HỢP 2: BẮT ĐƯỢC THÔNG TIN
    ...
    if (requiredChunks > remaining) {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Không đủ Chunk Quota',
            html: `
                <p>Cần <strong>${requiredChunks}</strong> chunks nhưng chỉ còn <strong>${remaining}</strong> chunks.</p>
                <hr style="margin: 15px 0; border-color: rgba(255,255,255,0.1);">
                <p style="font-size: 13px; color: #ffb86c;">Vui lòng đợi quota reset hoặc nâng cấp tài khoản.</p>
            `,
            confirmButtonText: 'Đã hiểu'
        });
        return false;
...
function updateRemainingChunksDisplay() {
    ...
    if (displayDiv && valueSpan) {
        if (remaining !== null) {
            // ============================================================
            // CÓ BẮT ĐƯỢC THÔNG TIN - Hiển thị số chunks
            // ============================================================
            ...
            // ============================================================
            // KHÔNG BẮT ĐƯỢC THÔNG TIN - Hiển thị "Unlimited"
            // ============================================================
            displayDiv.style.display = 'block';
            valueSpan.textContent = '∞ Unlimited';
            displayDiv.style.background = 'linear-gradient(135deg, rgba(80, 250, 123, 0.25) 0%, rgba(139, 233, 253, 0.25) 100%)';
            displayDiv.style.borderColor = 'rgba(80, 250, 123, 0.6)';
            displayDiv.style.borderWidth = '2px';
            valueSpan.style.color = '#50fa7b';
            valueSpan.style.fontWeight = '700';
            valueSpan.style.textShadow = '0 2px 4px rgba(0,0,0,0.3)';
            displayDiv.style.boxShadow = '0 4px 12px rgba(80, 250, 123, 0.3)';
            
            // Thêm tooltip giải thích
            displayDiv.title = 'Không tìm thấy thông tin "Remaining Previews" trên trang.\nCó thể tài khoản này không bị giới hạn chunk.';
        }
    }
}
...
if(LrkOcBYz_$AGjPqXLWnyiATpCI){
    ...
    if (window.batchState && window.batchState.isProcessing && window.batchState.currentFileName) {
        ...
        // Loại bỏ đuôi file .txt (nếu có)
        batchFileName = batchFileName.replace(/\.txt$/i, '');
        ...
        // Trả về tên file từ batch với đuôi .mp3
        return batchFileName + '.mp3';
    }
...
if (window.chunkBlobs && window.chunkBlobs.length > 0) {
    ...
    for (let i = 0; i < window.chunkBlobs.length; i++) {
        if (window.chunkBlobs[i] !== null && window.chunkBlobs[i] !== undefined) {
            ...
            addLogEntry(`⚠️ [MERGE] Chunk ${i + 1} bị null, bỏ qua`, 'warning');
        }
    }
    if (validBlobs.length > 0) {
        finalBlobs = validBlobs; // Chỉ dùng window.chunkBlobs nếu có dữ liệu
        addLogEntry(`✅ [MERGE] Ghép ${validBlobs.length} chunks theo đúng thứ tự`, 'info');
    }
} else {
    addLogEntry(`🔍 [MERGE] Fallback: Sử dụng ZTQj$LF$o`, 'info');
}
addLogEntry(`🔧 [MERGE] Tạo Blob từ ${finalBlobs.length} chunks...`, 'info');
// Tính tổng kích thước để kiểm tra
const totalSize = finalBlobs.reduce((sum, blob) => sum + (blob ? blob.size : 0), 0);
addLogEntry(`📊 [MERGE] Tổng kích thước: ${(totalSize / 1024 / 1024).toFixed(2)} MB`, 'info');
const InRdxToeqTDyPgDGZb=new Blob(finalBlobs,{'type':VCAHyXsrERcpXVhFPxmgdBjjh(0x1f5)}),BBNDYjhHoGkj_qbbbJu=URL[VCAHyXsrERcpXVhFPxmgdBjjh(0x1f0)](InRdxToeqTDyPgDGZb);addLogEntry(`✅ [MERGE] Đã tạo Blob URL thành công (${(InRdxToeqTDyPgDGZb.size / 1024 / 1024).toFixed(2)} MB)`, 'info');PEYtOIOW[VCAHyXsrERcpXVhFPxmgdBjjh(0x25c)]=BBNDYjhHoGkj_qbbbJu,PEYtOIOW[VCAHyXsrERcpXVhFPxmgdBjjh(0x1c8)]=i_B_kZYD(),zQizakWdLEdLjtenmCbNC[VCAHyXsrERcpXVhFPxmgdBjjh(0x1fb)][VCAHyXsrERcpXVhFPxmgdBjjh(0x1e1)]=VCAHyXsrERcpXVhFPxmgdBjjh(0x258),document[VCAHyXsrERcpXVhFPxmgdBjjh(0x1de)](VCAHyXsrERcpXVhFPxmgdBjjh(0x225))[VCAHyXsrERcpXVhFPxmgdBjjh(0x1fb)][VCAHyXsrERcpXVhFPxmgdBjjh(0x1e1)]=VCAHyXsrERcpXVhFPxmgdBjjh(0x258);addLogEntry(`🎨 [MERGE] Đang khởi tạo WaveSurfer...`, 'info');if(n_WwsStaC$jzsWjOIjRqedTG)n_WwsStaC$jzsWjOIjRqedTG[VCAHyXsrERcpXVhFPxmgdBjjh(0x26c)]();typeof WaveSurfer===VCAHyXsrERcpXVhFPxmgdBjjh(0x24d)&&await new Promise(dyvridmApUsyBfpYIHkxv=>setTimeout(dyvridmApUsyBfpYIHkxv,parseInt(0xf61)+Math.ceil(-parseInt(0x1e0))+-parseInt(0xb8d))),n_WwsStaC$jzsWjOIjRqedTG=WaveSurfer[VCAHyXsrERcpXVhFPxmgdBjjh(0x240)]({'container':VCAHyXsrERcpXVhFPxmgdBjjh(0x274),'waveColor':VCAHyXsrERcpXVhFPxmgdBjjh(0x26a),'progressColor':VCAHyXsrERcpXVhFPxmgdBjjh(0x228),'cursorColor':VCAHyXsrERcpXVhFPxmgdBjjh(0x20c),'barWidth':0x3,'barRadius':0x3,'cursorWidth':0x1,'height':0x64,'barGap':0x3}),n_WwsStaC$jzsWjOIjRqedTG[VCAHyXsrERcpXVhFPxmgdBjjh(0x1d5)](BBNDYjhHoGkj_qbbbJu),addLogEntry(`✅ [MERGE] WaveSurfer đã load xong`, 'info');n_WwsStaC$jzsWjOIjRqedTG['on'](VCAHyXsrERcpXVhFPxmgdBjjh(0x1d6),()=>{const Ipo_CDaCvNEfh=VCAHyXsrERcpXVhFPxmgdBjjh;XvyPnqSRdJtYjSxingI[Ipo_CDaCvNEfh(0x1c7)]='⏸️';}),n_WwsStaC$jzsWjOIjRqedTG['on'](VCAHyXsrERcpXVhFPxmgdBjjh(0x22d),()=>{const NdVplyNSVhdzFR=VCAHyXsrERcpXVhFPxmgdBjjh;XvyPnqSRdJtYjSxingI[NdVplyNSVhdzFR(0x1c7)]='▶️';});
...
}catch(FlhstZJmp_$Mvf){addLogEntry(`❌ [MERGE ERROR] Lỗi ghép file: ${FlhstZJmp_$Mvf.message}`, 'error');console.error('[MERGE ERROR] Chi tiết:', FlhstZJmp_$Mvf);}addLogEntry(`✅ [MERGE COMPLETE] Hàm ghép file đã hoàn thành`, 'success');}
...
async function uSTZrHUt_IC() {
    ...
    // === RESET KHI BẮT ĐẦU FILE MỚI (chunk 0) ===
    if (ttuo$y_KhCV === 0 && !window.isFinalCheck) {
        // Reset hoàn toàn các mảng khi bắt đầu file mới
        const currentTotalChunks = SI$acY.length;
        window.chunkBlobs = new Array(currentTotalChunks).fill(null);
        window.ZTQj$LF$o = new Array(currentTotalChunks).fill(null);
        window.chunkStatus = new Array(currentTotalChunks).fill('pending');
        window.failedChunks = [];
        window.totalChunksForCurrentBatch = currentTotalChunks;
        addLogEntry(`🔄 [FILE MỚI] Reset hoàn toàn các mảng (${currentTotalChunks} chunks)`, 'info');
    }

    // Khởi tạo hệ thống theo dõi chunk (nếu chưa có)
    ...
    // Đảm bảo mảng chunkStatus có đủ phần tử (chỉ mở rộng, không thu hẹp)
    // (Đã được reset ở chunk 0, nên ở đây chỉ cần đảm bảo đủ phần tử)
    ...
    try {
        ...
        // ANTI-DETECTION: Thêm delay ngẫu nhiên trước khi đặt text (1-3s)
            ...
            for (const TYRNWSSd$QOYZe of qcgcrPbku_NfOSGWmbTlMZNUOu[ndkpgKnjg(0x1db)]) {
                if (TYRNWSSd$QOYZe[ndkpgKnjg(0x217)] === 0x7fd * parseInt(-0x3) + 0xa02 + 0xdf6 && TYRNWSSd$QOYZe[ndkpgKnjg(0x1cd)](ndkpgKnjg(0x1f2))) {
                    ...
                    if (yEExghI && (yEExghI[ndkpgKnjg(0x20e)](ndkpgKnjg(0x1fa)) || yEExghI[ndkpgKnjg(0x20e)](ndkpgKnjg(0x26f)))) try {
                        ...
                        // ✅ KHỞI TẠO ĐƠN GIẢN: Chỉ tạo mới nếu chưa có
                        // (Đã có reset ở chunk 0, nên ở đây chỉ cần check undefined)
                        if (typeof window.chunkBlobs === 'undefined' || !Array.isArray(window.chunkBlobs)) {
                            window.chunkBlobs = new Array(totalChunks).fill(null);
                            addLogEntry(`🔧 Khởi tạo window.chunkBlobs với ${totalChunks} vị trí`, 'info');
                        }
                        ...
                        if (!Array.isArray(ZTQj$LF$o)) {
                            ZTQj$LF$o = new Array(totalChunks).fill(null);
                            addLogEntry(`🔧 Khởi tạo ZTQj$LF$o với ${totalChunks} vị trí`, 'info');
                        }
                    ...
                    ttuo$y_KhCV++;
                    // Delay ngẫu nhiên 2-4s trước khi xử lý chunk tiếp theo
                    setTimeout(uSTZrHUt_IC, Math.random() * 2000 + 2000);
                    return;
                }
            }
        }
    ...
    async function processSingleBatchFile(file, index, total) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                // ⚡ LƯU TÊN FILE HIỆN TẠI để i_B_kZYD() sử dụng
                window.batchState.currentFileName = file.name;
                // ===================================
                ...
                const checkCompletion = () => {
                    ...
                    if (finalResult && finalResult.style.display === 'block' && 
                        ...
                        // ⚡ Clear tên file hiện tại để không ảnh hưởng đến file tiếp theo
                        if (window.batchState) {
                            window.batchState.currentFileName = null;
                        }
                ...
                // ⚡ KHÔNG CÓ TIMEOUT - Chạy đến khi nào xong thì thôi
                checkInterval = setInterval(checkCompletion, 2000);
                
                // ❌ ĐÃ BỎ TIMEOUT - Không giới hạn thời gian xử lý nữa
                // Sẽ chạy liên tục cho đến khi checkCompletion() phát hiện xong
                ...
                // ⚡ Clear tên file hiện tại trong trường hợp lỗi
                if (window.batchState) {
                    window.batchState.currentFileName = null;
                }
